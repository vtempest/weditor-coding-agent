function convertZedTheme(zedTheme) {
...
async function fetchZedThemeJson(url) {
  try {
...
async function loadExtensionThemes(repoUrl, branch = "main", basePath = "") {
  const themes = [];
  ...
  try {
    ...
    const themeFiles = files.filter((f) => f.name.endsWith(".json"));
